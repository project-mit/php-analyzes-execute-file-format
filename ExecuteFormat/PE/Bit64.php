...
class Bit64 extends AbstractExecuteFormat
{
    ...
    public function getImageExportDescriptor(ImageNtHeaders &$ntHeader = null, array &$sectionHeader = null)
    {
        ...
        foreach ($sectionHeader as $index => $section)
        {
                ...
                $targetInfo->virtualAddress <  $section->virtualAddress + $section->misc['virtualSize'])
            {
                ...
                $targetDescriptor->addressOfFunctions -= $section->virtualAddress;
                $targetDescriptor->addressOfFunctions += $section->pointerToRawData;
                $targetDescriptor->addressOfNames -= $section->virtualAddress;
                $targetDescriptor->addressOfNames += $section->pointerToRawData;
                $targetDescriptor->addressOfNameOrdinals -= $section->virtualAddress;
                $targetDescriptor->addressOfNameOrdinals += $section->pointerToRawData;
    ...
    public function getListOfImportFunction(array $importDescriptorArray = null)
    {
        ...
        foreach ($importDescriptorArray as $key => $import)
        {
            ...
            for ($i = 0; ($importThunkData = $this->_streamio->read(8, $import->firstThunk + $i * 8)->toInteger()) !== 0; $i++)
            {
                ...
                if (($importThunkData & 0x8000000000000000) !== 0)
                {
                    $importByName->name = ($importThunkData & 0x00000000ffffffff);
                }
                else
                {
